...
| 字段 | 含义 |
| --- | --- |
| `cluster.zeromq.enabled` | 是否启用本节点 ZeroMQ listener 和 ZeroMQ peer 拨号能力。默认 `false`。 |
| `cluster.zeromq.bind_url` | 本节点 ROUTER socket 的本地监听地址，只允许 `tcp://host:port`。启用 ZeroMQ 时必填。 |
| `cluster.peers.url` | 远端可拨号地址。WebSocket 使用 `ws://` 或 `wss://`；ZeroMQ 使用 `zmq+tcp://host:port`。 |

`bind_url` 与 `cluster.peers.url` 不能混用：
...
1. 确认 `[cluster.zeromq].enabled` 和 `bind_url`。
...
- `enabled = true` 且缺失 `bind_url` 时拒绝启动。
...
- 三节点混合 WebSocket 与 ZeroMQ peer 时，membership 与动态路由不改变现有语义。
...
docker build -t turntf-notifier:zeromq-smoke .
```
...
5. 在 `Manager.Start` 中按配置启动 ZeroMQ listener。
...
- ZeroMQ v1 不做 CURVE。后续如果要让 ZeroMQ 直接跨不可信网络运行，必须单独设计 mTLS 隧道、CURVE 或其他链路加密方案。
