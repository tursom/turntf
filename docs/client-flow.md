...
中转规划会排除所有通向源节点及上一跳的边，避免直接回跳或经第三节点间接返回已访问节点；共享拓扑不被修改。TTL、转发权限、传输与桥接约束保持；不存在合法替代路径时仍拒绝转发。成功恢复记录 `mesh_route_loop_avoided`；入站丢弃记录 `mesh_packet_dropped`，包含原因、节点、包 ID、剩余跳数与流量类别，不包含业务内容。这不是任意长环的无环保证，TTL 仍是最终保护。

时间同步响应也由每条邻接独立任务发送，只保留最新待回应请求，避免慢写阻塞入站读取。接收时间在读取时记录，发送时间在任务开始发送时记录，将排队时间计入服务端处理时间；原校时验证和信任约束保持。

远程会话查询仍在入站处理阶段完成参数校验和本地会话快照；响应通过运行时的有界任务发送（最多 128 项），避免慢速出站写入阻塞同一连接后续的查询响应读取。接纳不等于送达，原查询期限仍有效；队列满时拒绝接纳，异步发送失败记录 `query_response_send_failed`，运行时关闭会取消并等待任务退出。该机制不改变业务 DATA 的顺序处理。

远程会话查询在调用者未提供截止时间时，使用覆盖排队、发送和响应等待的统一 3 秒预算；不再先等待 mesh 发送期限、再启动查询期限。调用者显式提供的截止时间保持原有语义。发送或响应超时均返回服务暂不可用，并清理 pending 查询，不把超时当作会话不存在。

集群拓扑更新在本地校验、应用后，按邻接连接异步广播；每条连接只有一个广播任务，每个来源只保留最新待发送版本。慢速出站连接不会阻塞入站读取循环，也不会阻止其他邻接接收更新。此队列仅处理完整拓扑公告，业务数据帧仍按原有顺序处理，连接移除或运行时关闭时停止广播任务。

集群传输的共享写锁支持排队期间的上下文取消：尚未获得写入权的调用不会关闭其他请求正在使用的连接。WebSocket 一旦发生实际写失败，立即关闭底层连接并唤醒读循环移除失效邻接，不再等待下一次心跳。TCP mTLS 仅在取得写入权后才启用取消时的流关闭保护；部分帧写入失败仍必须关闭整个流。

### 定向瞬时发送的并发边界

登录完成后，`/ws/realtime` 中同时指定 `delivery_kind = TRANSIENT` 和 `target_session` 的 `send_message`，以及只读 `resolve_user_sessions` 共用有界并发处理组。会话解析不再作为不同 relay 之间的顺序屏障，每个请求仍执行原有鉴权、校验和实时查询，不缓存结果。每连接两类请求合计最多 16 个在途请求；满额时读取循环保留当前一个已解码请求并等待名额，不增加 `busy` 错误，也不建立无界队列。WebSocket 原有 1MiB 帧上限不变。
...
定向会话校验优先只向 `target_session.serving_node_id` 查询该 recipient 的实时会话，并匹配完整 `session_ref`，不再遍历同一用户的其他 presence 节点或执行全网 fallback。不属于已知集群的节点视为目标不存在；已知节点查询失败仍返回原错误。未实现节点定向查询接口的 provider 继续使用原查询路径。公开的 `resolve_user_sessions` RPC 和非定向投递仍保持全节点发现语义。

这个范围内的请求执行、投递和响应可能重排，客户端必须按 `request_id` 关联响应；需要有序字节流的上层协议必须自行携带序号并重组。后续其他合法 protobuf 请求（包括 Ping、AckMessage、重复 Login、未定向发送和状态变更 RPC）等待此前两类在途请求完成后，仍按原串行路径处理。首帧登录、普通 `/ws/client`（包括 `transient_only=true`）、持久消息、HTTP 和 ZeroMQ 不启用此并发路径。
