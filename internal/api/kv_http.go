...
func (h *HTTP) kvPrincipal(w http.ResponseWriter, r *http.Request) (string, KVService, bool) {
	...
	return formatUserSubject(principal.User.Key()), h.service.kvService, true
}
...
func (h *HTTP) handleKVGrant(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.KVGrant(r.Context(), principal, r.PathValue("database"), r.PathValue("principal"), req.Permission)
	if err != nil {
		writeKVError(w, err)
		return
	}
