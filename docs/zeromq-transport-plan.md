...
- ZeroMQ 只作为集群节点间传输层试点，默认主链路继续使用 WebSocket。
...
- 不改客户端 WebSocket；`GET /ws/client` 仍只服务业务客户端。
...
配置新增可选的 `[cluster.zeromq]` 段。未配置或 `enabled = false` 时，行为必须与当前版本完全一致。
...
zeromq_curve_server_public_key = ""
```
...
| 字段 | 含义 |
| --- | --- |
| `cluster.zeromq.enabled` | 是否允许本节点使用 ZeroMQ transport。默认 `false`。 |
| `cluster.zeromq.bind_url` | 本节点 ROUTER socket 的本地监听地址，只允许 `tcp://host:port`。可选；为空时进入 outbound-only 模式。 |
| `cluster.zeromq.security` | ZeroMQ 安全模式，支持 `none` 或 `curve`，默认 `none`。 |
| `cluster.zeromq.curve.*` | CURVE 模式下的本节点 server/client key 与允许接入的 client public key 白名单。 |
| `cluster.peers.url` | 远端可拨号地址。WebSocket 使用 `ws://` 或 `wss://`；ZeroMQ 使用 `zmq+tcp://host:port`。 |
| `cluster.peers.zeromq_curve_server_public_key` | CURVE 模式下静态 `zmq+tcp` peer 的远端 server public key。 |
...
ZeroMQ 本机监听 URL 使用 `tcp` scheme，由 `[cluster.zeromq].bind_url` 单独校验：
...
- `normalizeZeroMQBindURL(raw string) (string, error)`：只用于 `[cluster.zeromq].bind_url`。
...
- membership 不主动把 `[cluster.zeromq].bind_url` 加入广告。
...
- `cluster.zeromq.enabled`。
- `cluster.zeromq.bind_url`。
- `cluster.zeromq.security` 与 `cluster.zeromq.curve.*`。
- `cluster.peers.zeromq_curve_server_public_key`。
...
1. 确认 `[cluster.zeromq].enabled`、`zeromq_mode` 和 `zeromq_listener_running`。
...
- `security = "curve"` 时，静态 `zmq+tcp` peer 缺少 `zeromq_curve_server_public_key` 应拒绝启动。
...
1. 设置 `cluster.zeromq.enabled = false`，或删除 `[cluster.zeromq]` 段。
