...
- `auth.reconnect_token_ttl_minutes` 默认 5 分钟；调大可减少长时间网络抖动后的 bcrypt 重认证，调小可缩短凭据暴露窗口。当前节点应用密码变更后会拒绝旧重连 token；跨节点失效受用户事件复制延迟影响。
...
`/ops/status` 的 `mesh` 字段用于解释当前节点的路由选择：
...
- `forwarded_packets_total{node_id,traffic_class,path_class}`：当前节点实际完成的 mesh 转发包数量，可用来确认某类流量是否在本节点走了 direct、same_transport_forward、cross_transport_bridge 或 native_relay。
...
- peer 长时间离线后重新接入，如果其需要的历史事件已经被对端裁剪，补拉会改走 snapshot 修复再继续追平；这时短时间看到 snapshot 请求增加属于预期。
