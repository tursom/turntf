...
TCP 建链后的活性检测复用 mesh `TimeSyncRequest/Response`，不是 TCP keepalive，也不是每次收到任意帧就刷新空闲期限。默认每 2 秒测量，每条邻接最多一个待应答请求；必须收到同一连接上匹配请求 ID 的应答。请求发出后 3 个测量周期（默认 6 秒）仍未应答，先给该物理邻接的成本加 200ms 惩罚，保留连接供后续探测；默认 30 秒无应答才在后续测量 tick 关闭连接，唤醒阻塞的 `Receive`，移除邻接并发布拓扑。计时使用本地单调时钟，不受 HLC 或墙钟校正影响。只读丢弃的对端即使让本地写入一直成功，也会被淘汰；其他业务帧不能续期。

从故障发生到发出下一次探测最多另等一个周期，超时检查有最多一个周期的 tick 粒度；阻塞发送另受 mesh 默认 5 秒发送期限约束。降权只在存在其他更低成本的同传输邻接时改变新数据的连接选择，已经固定到某连接的 stream epoch 仍需 Resume 后才能重选，并非固定 6 秒切换 SLA。TCP 发送探测失败也关闭连接。其他传输同样按探测超时关闭半开连接；WSS 沿用自身保活。连接取消或关闭时立即取消并等待其测量循环退出，清空待应答请求，不把该循环留到下次重试。

多条并行邻接沿用既有 `CostMs` 与 `JitterMs` 标签：`CostMs = RTT_EWMA + Jitter_EWMA + 质量惩罚`，`JitterMs` 保留实际 EWMA 抖动值；本地选路与拓扑 planner 均按两者之和比较，相当于 `RTT + 2×近期抖动 + 惩罚`。同一连接连续两次业务帧发送失败后增加 100ms，最多增加 300ms；成功发送业务帧清零，拓扑公告和探测帧不改变该计数，单次偶发失败不降权。业务发送和测量循环只标记成本变化，由已有拓扑发布 goroutine 合并处理，不等待拓扑广播或持久化。未响应探测的 200ms 惩罚在匹配且有效的应答到达后清除。惩罚只影响物理邻接和原有拓扑成本，不改变证书、身份或路由优先级：即使 TCP 成本高于 WSS，已建立的 TCP 直连仍优先于 WSS；只有 TCP 邻接断开才跨传输回退。
